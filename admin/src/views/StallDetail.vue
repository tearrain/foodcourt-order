...
import { getAdminStall, updateStall } from '@/api'
...
onMounted(async () => {
  try {
    const res: any = await getAdminStall(id.value)
    form.value = res.data || {}
  } catch {
    //
  } finally {
...
async function handleSave() {
  saving.value = true
  try {
    await updateStall(id.value, form.value)
    router.back()
  } catch {
    //
  } finally {
      ...
      <form @submit.prevent="handleSave" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex items-center gap-4 pt-6">
          ...
          <label class="flex items-center gap-2">
            ...
            <span class="text-sm">{{ t('stall.featured') }}</span>
          </label>
        </div>
